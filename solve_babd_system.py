...
def construct_babd_mshoot(size_y, size_z, size_p, M, A_tilde, C_tilde, H_tilde, b_tilde, B_0, B_n, V_n, b_n):
    sol = []
    for i in range(M):
        node_i = multiple_shooting_node.MultipleShootingNode(size_y, size_z, size_p)
        ...
        sol.append(node_i)
    node_n = multiple_shooting_node.MultipleShootingNode(size_y, size_z, size_p)
    node_n.set_B_n(B_n)
    node_n.set_H_n(V_n)
    node_n.set_b_n(b_n)
    sol.append(node_n)
